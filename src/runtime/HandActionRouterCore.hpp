...
inline void logUseRouteOnce() noexcept {
    ...
    if (logged.compare_exchange_strong(expected, true, std::memory_order_relaxed)) {
        __android_log_print(
            ANDROID_LOG_INFO,
            kActionDiagTag,
            "[ActionDiag] use route mainhand-first"
        );
    }
}
...
[[nodiscard]] UseRouteResult routeUseAction(
    ...
    // Java-like right click priority: MAINHAND owns the first native attempt.
    // OFFHAND is consulted only when mainhand returns PASS / not handled.
    {
